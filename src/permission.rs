...
impl BashSecurityValidator {
    ...
    pub fn describe_failures(&self, command: &str) -> String {
        ...
        if failures.is_empty() {
            return "No issues detected".into();
        }
...
impl std::fmt::Display for UserPermissionChoice {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let label = match self {
...
impl PermissionRule {
    ...
    pub fn default_rules() -> Vec<Self> {
        vec![
            Self::deny_tool_content("bash", "rm -rf"),
            Self::deny_tool_content("bash", "sudo *"),
            Self::allow_tool("read_file").with_path("*"),
        ]
    }
}
...
impl PermissionManager {
    pub fn try_new(mode: PermissionMode) -> Result<Self> {
        Self::try_new_with_rules(mode, PermissionRule::default_rules())
    }
            ...
            PermissionMode::Plan => {
                if is_write_tool(tool_name) {
                    Some(PermissionDecision {
                        behavior: PermissionBehavior::Deny,
                        reason: "Plan mode: write operations are blocked".into(),
                    })
                } else {
                    Some(PermissionDecision {
                        behavior: PermissionBehavior::Allow,
                        reason: "Plan mode: read-only allowed".into(),
                    })
                }
            }
            PermissionMode::Auto => {
                if tool_name == "read_file" || is_read_only_tool(tool_name) {
                    Some(PermissionDecision {
                        behavior: PermissionBehavior::Allow,
                        reason: "Auto mode: read-only tool auto-approved".into(),
                    })
                } else {
                    None
                }
